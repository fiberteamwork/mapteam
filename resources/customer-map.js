...
function drawCustomers(data, options){
    ...
    if (expandGroups) {
        ...
        const smallRandomOffset = (baseLat, baseLon, idx, total) => {
            ...
            const radius = metersToDeg(10 + Math.random() * 40); // 10-50m random
            return {
        ...
        data.forEach((customer, idx) => {
            ...
            const randPt = getRandomPointInWard(customer.city, customer.district, customer.ward);
            let c;
            if (randPt) {
                ...
                // No polygon found: fall back to snapCustomerToWard (centroid), then add random scatter
        ...
        // Group customers by polygon (city||district||ward). Aggregate all rows within the same ward/district into a single marker at the polygon centroid.
        idMap = new Map();
        ...
        data.forEach((customer, idx) => {
            ...
            const district = customer.district || '';
            const ward = customer.ward || '';
            const key = normalizeLocationKey(city, district, ward);
            const fallbackKey = key || `id::${String(customer.id || '')}`;
            if (!idMap.has(fallbackKey)) idMap.set(fallbackKey, []);
            idMap.get(fallbackKey).push(customer);
        });
        ...
        for (const [groupKey, members] of idMap.entries()) {
            ...
            // Multiple members in same ward/district: place single aggregated marker at random point within polygon
            ...
            let sampleCustomer = {
                ...
                resolvedBy: (rep.resolvedBy || 'original') + '|wardGroup'
            };
...
normalize(
    ...
    // When filtering by district or ward, expand groups to show all member markers
