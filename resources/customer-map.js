...
console.log('[CUSTOMER_MAP] loaded version 20260824.1');
...
function snapCustomerToWard(customer) {
    try {
        ...
        if (Number.isFinite(curLat) && Number.isFinite(curLon) && Math.abs(curLat) > 0.1 && Math.abs(curLon) > 100) {
            return customer;
        }
    ...
    if (expandGroups) {
        ...
        data.forEach((customer, idx) => {
            ...
            if (randPt) {
                ...
                const hasValidCoords = Number.isFinite(origLat) && Number.isFinite(origLon) && Math.abs(origLat) > 0.1 && Math.abs(origLon) > 100;
                if (hasValidCoords) {
                    // Customer has valid coords (from CSV or resolved earlier) — keep them with small scatter
                    origCoordsCount++;
                    c = Object.assign({}, customer);
