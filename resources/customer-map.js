            ...
            try {
                ...
                saveBtn.addEventListener('click', function() {
                    try {
                        ...
                        try {
                            // Build CSV using semicolon separator (match existing format)
                            ...
                            // Send to server endpoint that writes to data/customer.csv
                            fetch('save_customer_csv.php', {
                                method: 'POST',
                                ...
                                try {
                                    if (String(txt || '').trim() === 'OK') {
                                        alert('Koordinat disimpan dan diekspor ke data/customer.csv.');
                                    } else {
                                        alert('Koordinat disimpan tetapi ekspor gagal: ' + String(txt));
                                        console.warn('export response:', txt);
                                    }
                                } catch (e) { console.warn('post-export notify failed', e); }
                            }).catch(err => {
                                try { alert('Koordinat disimpan tetapi ekspor gagal: ' + (err && err.message ? err.message : String(err))); } catch (e) {}
                                console.error('export error', err);
                            });

                        } catch (e) { console.warn('export failed', e); try { alert('Koordinat disimpan. Namun ekspor gagal.'); } catch (ee) {} }
   ...
   uploadInput.addEventListener("change", function(event) {
       ...
       reader.onload = function(evt) {
           ...
           if (typeof contents === "string") {
               ...
               loadData(contents).then(() => {
                   // After successful load, export the uploaded CSV to server data/customer.csv
                   try {
                       // POST uploaded CSV to server using append mode
                       fetch('save_customer_csv.php?append=1', {
                           method: 'POST',
                           headers: { 'Content-Type': 'text/plain; charset=utf-8' },
                           body: contents
                       }).then(resp => resp.text()).then(txt => {
                           try {
                               if (String(txt || '').trim() === 'OK') {
                                   alert('CSV diunggah dan ditambahkan ke data/customer.csv');
                               } else {
                                   alert('CSV diunggah tetapi penambahan ke server gagal: ' + String(txt));
                                   console.warn('save_customer_csv response:', txt);
                               }
                           } catch (e) { console.warn('post-upload notify failed', e); }
                       }).catch(err => {
                           try { alert('CSV diunggah tetapi penambahan ke server gagal: ' + (err && err.message ? err.message : String(err))); } catch (e) {}
                           console.error('save_customer_csv error', err);
                       });
                   } catch (e) {
                       console.warn('export after upload failed', e);
                       try { alert('CSV diunggah tetapi terjadi kesalahan saat menyimpan ke server.'); } catch (ee) {}
                   }
               }).catch(err => {
