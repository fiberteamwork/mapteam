...
console.log('[CUSTOMER_MAP] loaded version 20260825.2');
...
function parseCustomerData(text){
    ...
    for (let index = 0; index < raw.length; index++){
        ...
        // Also treat truncated coords (e.g. lon=113 with <2 decimals) as invalid
        if (!invalidCoord) {
            ...
            const processChunk = () => {
                ...
                for (; i < end; i++) {
                    ...
                    if (!invalidCoord) {
                        const latDecimals = (String(lat).split('.')[1] || '').length;
                        const lonDecimals = (String(lon).split('.')[1] || '').length;
                        if (latDecimals < 2 || lonDecimals < 2) invalidCoord = true;
                    }
    ...
    if (expandGroups) {
        ...
        data.forEach((customer, idx) => {
            ...
            if (randPt) {
                ...
                // Require at least 2 decimal places — round numbers like lon=113 are truncated
                    ...
                    if (insidePolygon) {
                        ...
                        for (const ck of [nc2, normalize(stripAdminPrefixes(customer.city || ''))].filter(Boolean)) {
                            for (const dk of [nd2, normalize(stripAdminPrefixes(customer.district || ''))].filter(Boolean)) {
                                for (const wk of [nw2, normalize(shortWardName(customer.ward || '')), normalize((customer.ward || '').replace(/\s+/g, ''))].filter(Boolean)) {
