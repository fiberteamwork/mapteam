    ...
    if (msg.cmd === 'parse'){
        try{
            ...
            for (let i=0;i<raw.length;i++){
                ...
                let invalidCoord = (!Number.isFinite(lat) || !Number.isFinite(lon) || Math.abs(lat) < 1e-6 || Math.abs(lon) < 1e-6);
                if (!invalidCoord){
                ...
                if (invalidCoord){
                    ...
                    if (cityCoord){ lat = cityCoord.latitude; lon = cityCoord.longitude; resolvedBy='csvWard'; }
                    else {
                        // try wardMap from main thread
                        const w = getCoordinateFromMapIndex(wardMap, city, district, ward);
                        if (w){ lat = w.latitude; lon = w.longitude; resolvedBy='wardIndex'; }
